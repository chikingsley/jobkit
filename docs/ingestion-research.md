...
Use the local packaged `uv` Python project and `job-data/jobs.sqlite` as the source of truth.
...
- `job-data/jobs.sqlite` is about 20 MB, so storage size is not the hard problem.
...
Scheduled commands should run as `uv run --locked jobs ...` from `/home/simon/github/jobkit`.

...
That is the wrong shape for the product described in [jobkit-product-prd.md](jobkit-product-prd.md).
...
- `job-data/jobs.sqlite` has only 502 active ANESL rows, while the board notes describe a much larger
...
- Failed full refreshes died inside ANESL detail-page reads before `db.refresh_postings(...)` ran,
  leaving `job-data/jobs.sqlite` unchanged.

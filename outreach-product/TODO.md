...
- [ ] Validate the Jobs workspace at representative iPad-landscape and desktop
  ...
  and detail panes.
- [ ] Review and send the current five-position ANESL set after final recipient
  approval.
...
- [ ] Finish the contact workspace so every recruiter or intermediary shows all
  associated listings with the strongest match first and full descriptions
  available.
- [ ] Finish canonical-contact coverage so repeated recruiters and
...
- [ ] Add an opt-in sent-message scan that proposes voice preferences while the

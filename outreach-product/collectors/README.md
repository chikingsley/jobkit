...
Supported sources are `ajarn`, `anesl`, `eslcafe-modern` (or `eslcafe`), `seriousteachers`, and `tefl`. The default database is `.jobkit/jobs.sqlite`; the hosted runner explicitly supplies `../../job-search/job-data/jobs.sqlite`. Override it with `--db` or `JOBKIT_JOBS_DB_PATH`.

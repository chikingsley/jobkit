...
import {
  readSourceInventory,
  sourceDateEvidence,
} from "../../src/pipeline/01_ingest/ledger";
...
describe("collector inventory source", () => {
  ...
  test("carries collector source fields through without duplicating the listing", async () => {
    const directory = await mkdtemp(join(tmpdir(), "jobkit-fields-"));
    temporaryDirectories.push(directory);
    const databasePath = join(directory, "jobs.sqlite");
    const database = new Database(databasePath, { create: true, strict: true });
    database.exec(`
      ...
      INSERT INTO jobs (
        board,job_id,title,country,description,url,last_seen_at,raw_json
      ) VALUES (
        'anesl','20230920105526311','University Portuguese Teachers','China',
        'Age: Only From:20 To: 60 years old',
        'https://cafe.anesl.com/jobdetail.aspx?id=20230920105526311',
        '2026-07-21T00:00:00Z',
        json_object('fields',json_object(
          ...
          'Position ID','BJ6713',
          'Gender','  ',
          'body','the entire listing body text',
          'raw','the entire raw listing text',
          'company','ANESL',
          'apply_url','https://cafe.anesl.com/apply'
        ))
      );
    `);
    database.close();

    const [job] = readSourceInventory(databasePath).jobs;
    ...
    expect(job?.contactName).toBe("Mr. Corey Yang");
  });

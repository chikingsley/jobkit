...
import { cloudflareEmailsFromHtml } from "../../features/jobs/protected-email";
...
function definedFields(fields: Record<string, string | undefined>) {
  const kept: Record<string, string> = {};
  for (const [key, value] of Object.entries(fields)) {
    const trimmed = value?.trim();
    if (trimmed && !REDUNDANT_FIELD_KEYS.has(key)) {
      kept[key] = trimmed.slice(0, 4000);
    }
  }
  return kept;
}

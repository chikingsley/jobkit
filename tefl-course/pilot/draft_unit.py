...
The pipeline skeleton in miniature (see ../course-generation-pipeline.md): the model only
...
`unit-error-correction.md`, which must then pass the gate (dslop, patterns.js, audit, critic, owner)
before it counts as anything.

Run from the repo root:  uv run python tefl-course/pilot/draft_unit.py
"""
...
# The ONLY effectiveness claims the model may use, verbatim facts from curriculum-research.md §4.
CITATIONS_BLOCK = """\
...
def main() -> None:
    ...
    try:
        ...
        for title, brief, words in SECTIONS:
            ...
            print(f"drafted: {title} ({len(text.split())} words)", file=sys.stderr)  # noqa: T201
        ...
        print(f"wrote {OUT}", file=sys.stderr)  # noqa: T201
    finally:
        client.close()

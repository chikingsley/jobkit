...
- `dslop==0.2.2` installed as a uv dev dependency; verified it flags contrastives/em-dashes/negation. Known gap: no vocabulary list — covered by patterns.js.
- [conorbronsdon/avoid-ai-writing](https://github.com/conorbronsdon/avoid-ai-writing) vendored at `.claude/skills/avoid-ai-writing/` (SKILL.md catalog of 50+ patterns + the detector engine + tests). Verified the detector scores tier-1/tier-2 vocab, transitions, and chatbot closers on sample slop.
...
- The drafting prompts + `dslop.toml` + patterns.js threshold tuning.

# Module 13 Test — Assessment & Testing

Covers units 13.1 to 13.3. 16 points total; pass mark 70%. Section B re-asks questions from earlier modules: retrieving material again after a delay is part of how this course makes it stick.

## Section A — This module (9 points, 1 each)

**1. Which assessment quality is weak when two trained teachers give very different scores to the same speaking performance while using the same rubric?**

   a. Reliability, because the result changes substantially across raters
   b. Validity, because the task no longer measures spoken performance
   c. Washback, because students have not seen the scores in advance
   d. Authenticity, because more than one teacher observed the task

   *Answer: a. Reliability, because the result changes substantially across raters*
   *Anchor: "Reliability is the consistency of an assessment result under comparable conditions."*

**2. Which of the following best explains why a teacher should consider class size when selecting an assessment task?**

   a. Larger classes always require summative rather than formative assessment.
   b. Small-group formats need adjustment for larger classes
   c. Class size determines which level of Bloom's Taxonomy is appropriate for the lesson.
   d. Larger classes should only use traditional pen-and-paper assessment methods.

   *Answer: b. Small-group formats need adjustment for larger classes*
   *Anchor: "An assessment format designed for two groups of eight to ten students will need adjustment when thirty-five students are present"*

**3. A teacher whose lesson asked students to recall and define health vocabulary assigns a debate as the final assessment task. According to the unit, what is the core problem with this choice?**

   a. Debates are not listed as a valid form of alternative assessment.
   b. The debate produces a summative score rather than formative feedback.
   c. The debate requires unequipped Evaluate-level thinking.
   d. Debates are only appropriate when students have high intermediate proficiency.

   *Answer: c. The debate requires unequipped Evaluate-level thinking.*
   *Anchor: "Assigning a debate as the assessment task would place students at the Evaluate level, asking them to perform cognitive work they have not yet been guided through"*

**4. According to the unit, which of the following is identified as a reason why alternative assessment can increase learner motivation?**

   a. It removes the need for any formal grading system
   b. Learners compete with peers and receive ranked scores
   c. It reduces the amount of work teachers must prepare outside class
   d. Real language use and visible personal progress

   *Answer: d. Real language use and visible personal progress*
   *Anchor: "When learners use the language for real purposes and see their own progress clearly, their motivation to continue using English may increase."*

**5. The unit describes two different physical storage arrangements for portfolios observed in classrooms. What was the storage method used specifically by older students?**

   a. Large shelf-stored binders collected on arrival
   b. Folders hung on a dedicated classroom display board
   c. Large plastic envelopes kept in bins on the floor
   d. Cardboard boxes kept in a shared school storage room

   *Answer: a. Large shelf-stored binders collected on arrival*
   *Anchor: "Older students used large binders that sat on shelves in the classroom; students picked up their binders when they arrived."*

**6. In the unit's King Lear dramatization example, which of the following accurately lists language skills the unit says a student like Priya must use during the performance?**

   a. Writing, editing, memorizing lines, and presenting to an examiner
   b. Listening, reading, pronunciation, fluency, and dramatic expression
   c. Speaking spontaneously, taking notes, summarizing, and debating
   d. Reading silently, translating, writing a reflection, and answering comprehension questions

   *Answer: b. Listening, reading, pronunciation, fluency, and dramatic expression*
   *Anchor: "listen carefully to hear the play read correctly, read for comprehension, sustain reading fluency, attend to pronunciation, and deliver her lines with dramatic expression."*

**7. According to the unit, what specific problem arises when a gap-fill distractor can be eliminated through elementary subject-verb agreement rather than knowledge of the target form?**

   a. The item becomes double-barreled and tests two skills simultaneously
   b. The item produces negative washback by penalizing correct responses
   c. The item is reduced to a guessing exercise with an obvious answer
   d. The item fails the alignment check because it targets an unstated objective

   *Answer: c. The item is reduced to a guessing exercise with an obvious answer*
   *Anchor: "Implausible distractors reduce an item to a guessing exercise with an obvious answer"*

**8. The unit argues that a teacher who relies exclusively on knowledge-based test items will consistently do which of the following?**

   a. Overestimate students whose metalinguistic awareness exceeds their spoken production
   b. Align assessment with communicative learning goals more efficiently
   c. Produce positive washback by rewarding accurate grammar recognition
   d. Underestimate students whose communicative ability outpaces their metalinguistic awareness

   *Answer: d. Underestimate students whose communicative ability outpaces their metalinguistic awareness*
   *Anchor: "underestimate students whose communicative ability outpaces their metalinguistic awareness"*

**9. According to the unit's test-writing workflow, what is the correct order for the first and fourth steps?**

   a. State goals first; write the marking guide fourth
   b. Write the marking guide first, then identify the learning goals
   c. Draft the items first, then write the learning goals to match them
   d. Match item types to goals first, then share the assessment criteria with students

   *Answer: a. State goals first; write the marking guide fourth*
   *Anchor: "The first step is to write out the lesson's learning goals in plain language before touching a single test item"*

## Section B — Review (2 points, 1 each)

**10. Daniel objects that 'I am seeing the dentist tomorrow' is a present tense form and cannot refer to the future. According to the unit, what is the root cause of this confusion?**

   a. Daniel has not yet studied the present continuous aspect.
   b. Calling 'will + infinitive' a tense implies it is the only grammatical mechanism for future time.
   c. The teacher failed to explain that English has three morphological tenses.
   d. Daniel's first language does not distinguish tense from aspect.

   *Answer: b. Calling 'will + infinitive' a tense implies it is the only grammatical mechanism for future time.*
   *Anchor: "unit 2.3 quiz"*

**11. Describe the full sequence of skills a student moves through during the free-write activity, from selecting a picture to the teacher's review of the finished notebook entry.**

   *Answer: The student reads a magazine image, makes a writing choice, writes a text (writing), reads that text aloud (speaking), while the peer listens with a worksheet task (listening); the written page is then placed in a notebook for the teacher to review later.*
   *Anchor: "unit 8.5 quiz"*

## Section C — Scenario (5 points)

**12. Mr. Osei has spent three weeks running speaking activities with his intermediate class. His students — Carlos, Daniel, Wei, Fatima, Yuki, and Priya — have been practicing leading small-group discussions, giving opinions, and including quieter classmates. At the end of the unit, Mr. Osei wants to find out how well each student can actually perform these skills. He also wants to give students feedback they can act on before he records a final grade. He drafts a written grammar quiz on discourse markers as his end-of-unit assessment because it is quick to mark. A colleague, Ms. Reyes, suggests he reconsider. What should Mr. Osei do instead, and why? Design a brief assessment plan (two components) that addresses both his summative and his formative purposes, and explain how each component aligns with what was taught.**

   *Answer: Mr. Osei should replace the grammar quiz with a two-component assessment that matches his learning goals. First, he should use a performance-based task — for example, having each student (Carlos, Daniel, Wei, Fatima, Yuki, or Priya) lead a small-group discussion while he listens and completes a teacher rubric. The rubric score becomes the summative record: a defensible final grade reflecting each student's communicative achievement. This aligns directly with what was taught because the assessment task and the learning goal point toward the same outcome — spoken discussion skill — rather than written knowledge of grammar labels. A grammar quiz, by contrast, would test knowledge about discourse markers, not the ability to deploy them under real communicative pressure; a student like Carlos might use them fluently in speech yet blank on a written definition, so the quiz would misrepresent his actual ability. Second, Mr. Osei should have classmates complete peer assessment worksheets during the same discussion. These forms are explicitly not a formal grade; they give the discussion leader structured feedback from the group's perspective — for instance, whether Fatima included everyone or whether Wei's contributions were clear — information the student can use to improve before or alongside the summative judgment. To ensure fairness and clarity, Mr. Osei should show students both the teacher rubric and the peer worksheet in advance, so they know precisely how they will be evaluated and can articulate each criterion in their own words. This transparency satisfies the clarity requirement for a well-designed assessment task. Together, the two components serve entirely different purposes while sitting side by side in the same lesson: one produces a final record, the other produces actionable feedback.*
   *Anchor: "the assessment task and the learning goal point toward the same outcome"*

### Scoring guidance

**5:** Complete, accurate diagnosis and feasible action, justified with the module's relevant principle(s) and applied to the named context.
**3-4:** Sound core response with one missing, underdeveloped, or weakly justified element.
**1-2:** Partial recognition of the issue, but the action is vague, incomplete, or poorly connected to module content.
**0:** Blank, irrelevant, or incompatible with the module's central principle.

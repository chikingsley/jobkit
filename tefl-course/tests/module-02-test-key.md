# Module 2 Test — English Grammar for Teachers I

Covers units 2.1 to 2.5. 22 points total; pass mark 70%. Section B re-asks questions from earlier modules: retrieving material again after a delay is part of how this course makes it stick.
...
**2. What is the grammatical role of 'in front of' in the sentence 'The car parked in front of the gate'?**
   ...
   *Answer: c. A complex preposition functioning as a single unit*
   *Anchor: "Complex prepositions consist of two or three words functioning as a single unit: in front of, next to, because of"*
...
**5. In the sentence 'The list on the whiteboard confused Daniel,' what grammatical role does the prepositional phrase 'on the whiteboard' perform?**
   ...
   *Answer: b. It modifies the noun 'list'.*
   *Anchor: "the prepositional phrase 'on the whiteboard' modifies the noun 'list'"*
...
**7. According to the unit, what is the key difference between the perfect simple 'He has run five kilometres today' and the perfect continuous 'He has been running'?**
   ...
   a. A. The perfect simple refers to present time while the perfect continuous refers to past time.
   b. B. The perfect simple foregrounds a completed, measurable result while the perfect continuous foregrounds the activity as an extended process.
   c. C. The perfect simple uses a past participle while the perfect continuous uses a present participle, making them different tenses.
   d. D. The perfect continuous can only be used when the activity has fully stopped, whereas the perfect simple implies it is still ongoing.

   *Answer: b. B. The perfect simple foregrounds a completed, measurable result while the perfect continuous foregrounds the activity as an extended process.*
...
**8. The unit uses the image of a horizontal line representing time to explain aspect. Which of the following correctly matches the two aspects to their visual representations?**
   ...
   a. A. Simple aspect = a segment of the line showing spread; continuous aspect = a single dot with no visible interior.
   b. B. Simple aspect = a bridge between two points on the line; continuous aspect = a single dot.
   c. C. Simple aspect = a single dot with no visible interior; continuous aspect = a segment showing the action spreading across it.
   d. D. Simple aspect = a segment showing duration; continuous aspect = a bridge connecting two time points.

   *Answer: c. C. Simple aspect = a single dot with no visible interior; continuous aspect = a segment showing the action spreading across it.*
...
**9. The unit argues that a learner of English whose first language is Mandarin faces a particular challenge with the English tense-aspect system. What is that challenge, according to the unit?**
   ...
   a. A. Mandarin marks both tense and aspect through verb inflection, so learners must unlearn inflectional habits.
   b. B. Mandarin does not inflect verbs for tense and marks aspect with particles, so the learner needs explicit explanation of what the English system is doing.
   c. C. Mandarin has more tenses than English, causing learners to over-differentiate time reference.
   d. D. Mandarin uses modal auxiliaries for future time in the same way English does, creating false transfer errors.

   *Answer: b. B. Mandarin does not inflect verbs for tense and marks aspect with particles, so the learner needs explicit explanation of what the English system is doing.*
...
**13. According to the unit, what is the term for the influence a learner's first language exerts on their English production?**
   ...
   *Answer: b. Transfer*
   *Anchor: "The term for this influence is transfer. Transfer occurs when a learner applies an L1 rule to English"*
...
**14. The unit describes a pattern where learners who have learned that English requires articles begin inserting 'the' where zero article is required. What is this pattern called?**
   ...
   *Answer: c. Overcorrection*
   *Anchor: "Overcorrection produces a different pattern. Having learned that English requires articles, some learners begin inserting 'the' where zero article is required."*
...
**17. What does the unit recommend a teacher do with wait time after asking a question?**

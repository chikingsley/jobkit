"""Build module assessments and the final exam from per-unit question banks.
...
2. MODULE ASSESSMENTS (one per module). Sections: A) this module's material, 3 bank questions per
...
Run from `tefl-course/`:

    uv run tefl-course-assessments bank all     # build/refresh question banks
    uv run tefl-course-assessments assemble     # write module assessments + final exam
"""
...
from tefl_course.checks import strip_duplicate_option_label
from tefl_course.llm import SuperwhisperClient
from tefl_course.paths import PROJECT_ROOT
from tefl_course.pipeline import build
from tefl_course.pipeline.registry import UNITS, Unit
...
def _key_text(q: dict[str, object]) -> str:
    ...
    if isinstance(options, list):
        ...
        for j, opt in enumerate(options, 1):
            label = chr(96 + j)
            raw = str(opt).strip()
            clean = strip_duplicate_option_label(raw, label)
            candidates = {raw.lower(), clean.lower()}
            if any(
                candidate == low or candidate in low or low in candidate for candidate in candidates
            ):
...
def cmd_scenarios() -> None:
    ...
    try:
        for module in sorted(MODULE_TITLES):
            cache = ASSESSMENTS_DIR / f"_scenario-M{module}.json"
            if cache.exists():
                continue
        ...
        for j, (lo, hi, label) in enumerate(spans, 1):
            cache = ASSESSMENTS_DIR / f"_scenario-final-{j}.json"
            if cache.exists():
                continue
...
def _preflight() -> None:
    ...
    for module in sorted(MODULE_TITLES):
        path = ASSESSMENTS_DIR / f"_scenario-M{module}.json"
        if not path.exists():
            missing.append(str(path))
    for j in (1, 2, 3):
        path = ASSESSMENTS_DIR / f"_scenario-final-{j}.json"
        if not path.exists():
            missing.append(str(path))
    if missing:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="tefl-course-assessments",
        description="Build module assessments and the final exam.",
    )

...
Run from `tefl-course/`. ``SUPERWHISPER_API_BASE`` + ``SUPERWHISPER_API_KEY`` must be set
(env or ``.env``):

    uv run tefl-course-transcribe-archive
"""
...
from tefl_course.pipeline.turns import Turn, to_markdown, words_to_turns

ARCHIVE = PROJECT_ROOT / "sources" / "las-vegas-teaching-archive"
...
def _label_speakers(
    ...
    try:
        ...
        print(f"  labelling failed ({exc}); using raw ids", file=sys.stderr)
        return "", {}
...
def main() -> None:
    ...
    print(f"done: {rendered} rendered, {skipped} skipped, {failures} failed", file=sys.stderr)
